...
export async function POST(request: NextRequest) {
  try {
    ...
    const maxBrandKits = planLimits[organization.plan] || 1;
    ...
    const { name, logoPublicId, primaryColor, secondaryColor, fontFamily } =
      await request.json();
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const { id, name, logoPublicId, primaryColor, secondaryColor, fontFamily } =
      await request.json();
    ...
    const updatedBrandKit = await prisma.brandKit.update({
      ...
      data: {
        ...
        logoPublicId:
          logoPublicId !== undefined ? logoPublicId : brandKit.logoPublicId,
        primaryColor:
          primaryColor !== undefined ? primaryColor : brandKit.primaryColor,
        secondaryColor:
